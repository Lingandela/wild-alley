...
function overlay(mat: THREE.MeshLambertMaterial) {
  const m = mat.clone();
  m.depthTest = false;
  m.depthWrite = false;
  return m;
...
function SlotCard({
  ...
  const edge = useMemo(() => overlay(retro(card.type === "sabotage" ? "#3a1818" : "#d8c8a0")), [card.type]);
...
function Hand({ side, skin, knuckle, nail }: { side: 1 | -1; skin: THREE.Material; knuckle: THREE.Material; nail: THREE.Material }) {
  return (
    <group
      ...
      rotation={[0.22, side * -0.22, side * 0.18]}
    >
      ...
      {[-0.028, -0.01, 0.008, 0.024].map((fy, i) => (
...
export function Wallet({
  ...
  const leather = useMemo(() => overlay(retroMapped(leatherMap, "#d4b08a", { snap: false })), [leatherMap]);
  const suede = useMemo(() => overlay(retroMapped(suedeMap, "#a05048", { snap: false })), [suedeMap]);
  const stitch = useMemo(() => overlay(retro("#c4a070")), []);
  const brass = useMemo(() => overlay(retro("#c47a3a", { emissive: "#c47a3a", emissiveIntensity: 0.28 })), []);
  const pocket = useMemo(() => overlay(retro("#4a281c")), []);
  const plastic = useMemo(() => overlay(retro("#d8e8f0", { transparent: true, opacity: 0.22 })), []);
  const skin = useMemo(() => overlay(retro("#d2a078")), []);
  const knuckle = useMemo(() => overlay(retro("#c49068")), []);
  const nail = useMemo(() => overlay(retro("#efe0cc")), []);
